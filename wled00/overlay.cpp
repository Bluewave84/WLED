...
{
  uint32_t hh_mm_color = 0xffffda33;
  uint32_t ss_hand_color = 0xffff510d;
  uint32_t marker_color = 0xff382717;
  ...
  if (analogClockSecondsTrail)
  {
    ...
    {
      strip.setRange(analogClock12pixel, overlayMax, ss_hand_color);
      strip.setRange(overlayMin, secondPixel, ss_hand_color);
    } else
    {
      strip.setRange(analogClock12pixel, secondPixel, ss_hand_color);
    }
  }
  if (analogClock5MinuteMarks)
  {
    for (byte i = 0; i <= 12; i++)
    {
      ...
      strip.setPixelColor(pix, marker_color);
    }
  }
  if (!analogClockSecondsTrail) strip.setPixelColor(secondPixel, ss_hand_color);
  strip.setPixelColor(minutePixel, hh_mm_color);
  strip.setPixelColor(hourPixel, hh_mm_color);
}
